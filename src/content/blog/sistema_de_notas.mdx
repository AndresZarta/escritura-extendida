...
En el [post introductorio](/blog/init_escritura_extendida) al sitio, hable brevemente de mi experiencia con el sitio de notas de Andy Matuschak. 
...
Al leer su sitio por primera vez, sentí que se trataba de algo muy diferente a consultar un archivo tradicional, sino como atravesar un espacio de pensamiento en expansión donde cada nota puede actuar tanto como una pieza individual de pensamiento o como una conexión hacia otras ideas, y donde las asociaciones entre notas, y no los índices jerárquicos cerrados, gobiernan la navegación conceptual. 
...
En Obsidian, las notas se escriben utilizando [Markdown](/notes/markdown), y es posible interconectarlas mediante enlaces internos, tanto usando la sintaxis de “wikilinks” (`[[Nombre de la nota]]`) como mediante enlaces Markdown estándar (`[texto](ruta-al-archivo.md)`), lo que crea una red de ideas navegable y exploratoria.  ￼
...
Dada mi relación prolongada con Obsidian (ya conocía su sintaxis, su forma de enlazar ideas y la sensación de escribir en un entorno pensado para el pensamiento) usar el loader de Obsidian, pude conservar mi vocabulario de siempre (doble corchete, enlaces internos, etiquetas), y hacer que Astro se adaptara a ese idioma. El loader de [Obsidian](/notes/obsidian) me permite continuar escribiendo en mi herramienta habitual, mantener wikilinks y assets en su lugar, y a la vez exponer esa red de notas como datos consumibles en Astro.
...
Aquí `getCollection('notes')` obtiene todas las notas y descarta borradores; el componente `Base` recibe título y descripción para la cabecera, y se genera una lista simple donde cada `note.id` construye la ruta estática `/notes/{id}` que enlaza a la nota renderizada.
